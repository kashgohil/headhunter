...
`/settings/data` exports application data as a versioned JSON backup, including original descriptions, submitted snapshots, evidence, contacts, interview notes, reminders, and audit history. Referenced external attachments are not bundled. Google Calendar credentials, OAuth state, cached provider events, and provider links are excluded; restoring an ordinary export leaves that integration disconnected. Downloads are readable private data and use `private, no-store` responses; the UI explains their sensitivity.
...
## Read-only calendar integration (ISSUE-284)

Google Calendar uses calendar-list and event read-only scopes. The user selects calendars, requests each sync, reviews candidate events, and explicitly links an event to an interview or opportunity. Candidate events never create job records automatically. Sync stores bounded event fields, provider identity, recurrence identity, original instance time, provider timestamps, and time zones. Linked reschedules and cancellations update the authoritative interview timing/status while preserving manual content and completed rounds.

Credentials are encrypted with an installation key, OAuth state is hashed and expires, and OAuth responses disable caching and referrer propagation. Ordinary exports omit the private integration graph. Disconnect attempts provider revocation, removes local provider data, and retains interview records. Sync errors and authentication expiry preserve cached events and notes and remain visible for retry or reconnect. Refresh is manual; background delivery and all calendar writes are outside the integration. Setup and rotation details are in [Google Calendar integration](calendar-integration.md). Hosted callbacks use the ISSUE-285 owner session, canonical-host, and HTTPS boundary.

## Private hosted access (ISSUE-285)

Hosted mode targets one Docker container on a private Linux host behind Caddy HTTPS, with SQLite on an operator-attested encrypted persistent directory. A single scrypt-hashed owner password creates a signed twelve-hour cookie backed by a revocable server-side session digest. The request proxy applies canonical-host, HTTPS, origin, and optimistic session checks. Request-time workspace layout checks, all Server Actions, and every private route handler repeat live owner authorization; guessed identifiers do not bypass this boundary. Local mode remains available only through loopback hostnames.

Production startup validates secrets and storage, probes the mounted directory, applies migrations, and refuses to serve after failure. The public health response discloses only readiness. SQLite sidecars are owner-only; sensitive records rely on encrypted storage, Calendar credentials retain field encryption, and hosted exports/recovery copies add authenticated backup encryption. Session records never enter backups. Deployment, key rotation, retention/deletion, disaster recovery, and schema-aware rollback are specified in [Private hosted deployment](hosted-deployment.md). This work prepares deployment but does not deploy the application or connect an account.
...
## Cohort funnel and improvement engine (ISSUE-250)

`/analytics` shows 30-day, 90-day, and all-time application cohorts. Cohort membership uses the earliest recorded Applied event or submission, including explicit waivers. Each milestone shows its count, cohort denominator, percentage, and linked applications. Events before the first application or in the future are excluded; repeated transitions count once and custom stages map to their standard categories. Downstream milestones are not inferred. Zero denominators show an undefined rate; small samples and immature outcomes carry explicit caveats. Segmentation now covers role family, seniority, industry, company size, source, referral, resume strategy, location, compensation, posting age, actual preparation effort, UTC application weekday, and all historical fit dimensions. Counts/rates/sample sizes/windows remain together; missing values form an explicit group and every segment links to its applications. First-submission snapshots provide historical attribution; later submissions and future fit analyses cannot rewrite it. Missing snapshot metadata is labelled as current. User annotations fill unrecorded dimensions without changing job or resume snapshots.

Saved experiment plans define exactly one variable, two segment values, an independent target segment, held-constant conditions, dates, and a success milestone with equal 7–90-day observation periods. Only one experiment can run at once. Immature applications are excluded from rate denominators and reported separately; outcomes after each application’s observation period are excluded. Completion waits for the full period and freezes the result. Plan/result immutability is enforced by database triggers; revision checks and transactional audit records protect edits. Migration 0018 adds analytics annotations and experiments, automatically included in workspace backups. Comparisons remain descriptive correlations with explicit small-sample and non-causal caveats. See `docs/issue-250-validation.md` for validation and interpretation boundaries.
...
## Weekly review (ISSUE-251)

`/reviews` creates one review per completed Monday–Sunday week in UTC. Week boundaries are start-inclusive and end-exclusive; invalid, non-Monday, current, and future weeks are rejected. Reopening a week returns its existing review. The history is paginated and separates drafts from reviewed entries.

Facts retain source labels, links, counts, and the week's recorded stage transitions in an immutable SQLite snapshot. They cover captured jobs, distinct applications (submission or Applied event, including waivers), outbound interactions, inbound responses, non-cancelled interview rounds, recorded outcomes, progression, closure, and missed task dates. Repeated application/stage records do not inflate distinct-job counts. Custom stages map to their standard category when determining forward progress. Scheduled interviews do not establish attendance. Outbound logs currently do not distinguish first contact from follow-ups, and the review says so.

Missed tasks use recorded UTC due dates and completion times: completion by the end of the due date is on time. Tasks created after the reviewed week are excluded. Current stalled opportunities, action/deadline risks, and data-quality reminders are captured separately and labelled with snapshot creation time. They include dismissed and snoozed reminders. Neither current risks nor subsequently edited task dates are presented as reconstructed historical state.

Rule-based interpretations and a suggested single-variable experiment remain explicitly provisional. Activity counts are not conversion cohorts; small-sample and attribution caveats prevent causal or resume-performance claims. Reflections, interpretations, up to two experiment ideas, next-week priorities/deferrals, and available/planned hours are editable. The suggested plan uses rough allowances bounded by the saved weekly budget (or a disclosed three-hour placeholder). Users adjust these estimates; planned hours cannot exceed available hours. Completion requires a reflection and plan. Saved plans do not create tasks or initiate communication.

Snapshot/week fields are protected by a database trigger. Edits use optimistic revision checks, retain unsaved input on errors, and reject stale-tab overwrites. Creation and edits are audited transactionally; audit failure rolls back the operation. Reviews participate in workspace backups and their notes/plans are included in global search. Migration 0017 adds the review table, unique week index, and immutability trigger. Existing same-schema backup restrictions apply.

Validation covers 101 automated domain/SQLite tests, lint, TypeScript, a webpack production build, and isolated desktop/mobile Chromium workflows for creation, completion, validation, capacity, repeated reopen, stale-tab protection, search, and calendar accessibility. Snapshot immutability, restore, and audit rollback are tested against the full migration history. Email/calendar integrations and controlled experiment tracking remain outside this feature.
