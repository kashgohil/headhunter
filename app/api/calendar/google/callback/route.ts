...
export async function GET(request: Request) {
  ...
  if (input.get("error")) return Response.redirect(settingsUrl(request, { error: "Google Calendar access was not approved." }), 307);
  if (!code || !state) return Response.redirect(settingsUrl(request, { error: "Google returned an incomplete authorization response. Start again." }), 307);
  try {
    await completeGoogleCalendarConnection(code, state);
    return Response.redirect(settingsUrl(request, { connected: "1" }), 307);
  } catch (error) {
    ...
    return Response.redirect(settingsUrl(request, { error: message }), 307);
  }
}
