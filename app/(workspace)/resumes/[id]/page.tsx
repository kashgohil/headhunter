...
import { ArrowLeft, Check, Download, FileLock2, Lock, RefreshCw, ShieldCheck, Unlock, X } from "lucide-react";
...
function ReviewMetadata({ reason, requirement, evidenceIds, evidence, confidence, risk }: { reason: string; requirement: string; evidenceIds: string[]; evidence: Map<string, { label: string; state: string }>; confidence: string; risk: string }) {
  return <details className="mt-4 rounded-md border border-border bg-muted/35"><summary className="cursor-pointer list-none px-4 py-3 text-xs font-semibold outline-none focus-visible:ring-2 focus-visible:ring-ring/30">Why this changed</summary><dl className="grid gap-3 border-t border-border px-4 py-4 text-xs leading-5 sm:grid-cols-2"><div><dt className="font-medium text-foreground">Reason</dt><dd className="mt-1 text-muted-foreground">{reason}</dd></div><div><dt className="font-medium text-foreground">Requirement addressed</dt><dd className="mt-1 text-muted-foreground">{requirement}</dd></div><div><dt className="font-medium text-foreground">Evidence</dt><dd className="mt-1 space-y-1 text-muted-foreground">{evidenceIds.length ? evidenceIds.map((id) => <Link key={id} href={`/career-profile#evidence-${id}`} className="block underline decoration-border underline-offset-2 hover:text-foreground">{evidence.get(id)?.label || "Evidence record"} · {evidence.get(id)?.state.replace("_", " ") || "recorded"}</Link>) : "No supporting evidence linked"}</dd></div><div><dt className="font-medium text-foreground">Assessment</dt><dd className="mt-1 flex gap-2"><Badge variant="outline">{confidence} confidence</Badge><Badge variant={risk === "low" ? "signal" : "outline"}>{risk} risk</Badge></dd></div></dl></details>;
}
  ...
  return (
    <div className="mx-auto w-full max-w-[1440px] px-5 py-8 sm:px-8 lg:px-10 lg:py-10">
      ...
      <div className="grid gap-8 pt-8 xl:grid-cols-[minmax(0,680px)_minmax(480px,1fr)]">
        <aside className="xl:sticky xl:top-8 xl:self-start"><div className="mb-3 flex items-center justify-between"><p className="text-sm font-semibold">Document preview</p><p className="font-mono text-xs uppercase tracking-wider text-muted-foreground">{detail.resume.template}</p></div><ResumePreview snapshot={snapshot} /></aside>
