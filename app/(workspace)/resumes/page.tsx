    ...
    <div className="mx-auto w-full max-w-6xl px-5 py-8 sm:px-8 sm:py-12 lg:px-12 lg:py-14">
      ...
      <section className="grid gap-6 py-8 lg:grid-cols-[minmax(0,1fr)_340px]" aria-labelledby="drafts-heading">
        ...
        <Card className="h-fit">
          ...
          <CardContent><form action={createTailoredResumeAction} className="space-y-4"><div><label className="text-sm font-medium">Base resume</label><Select name="baseResumeId" required><SelectTrigger className="mt-2"><SelectValue placeholder="Choose a profile" /></SelectTrigger><SelectContent>{studio.profiles.map((item) => <SelectItem key={item.id} value={item.id}>{item.name}</SelectItem>)}</SelectContent></Select></div><div><label className="text-sm font-medium">Target job</label><Select name="jobId" required><SelectTrigger className="mt-2"><SelectValue placeholder="Choose a job" /></SelectTrigger><SelectContent>{jobs.map((job) => <SelectItem key={job.id} value={job.id}>{job.title} · {job.company}</SelectItem>)}</SelectContent></Select></div><Button className="w-full" disabled={!studio.profiles.length || !jobs.length}><FilePlus2 /> Generate review draft</Button>{!studio.profiles.length || !jobs.length ? <p className="text-xs leading-5 text-muted-foreground">You need at least one base resume and one captured job.</p> : null}</form></CardContent>
        </Card>
      </section>
