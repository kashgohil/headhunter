...
function EvidenceState({ item }: { item: EvidenceItem }) {
	return (
		<div className="flex flex-wrap items-center gap-2">
			<Badge
				variant={item.verificationState === "verified" ? "signal" : "outline"}
				className={cn(
...
function Provenance({ item }: { item: EvidenceItem }) {
	return (
		<p className="text-xs leading-5 text-muted-foreground">
			...
			{item.sourceLabel ? ` · ${item.sourceLabel}` : " · No source note"}
		</p>
	);
}
	...
	if (locked)
		return (
			<p className="text-xs text-muted-foreground">
				Unlock this item before editing it.
			</p>
		);
...
function ReadinessPanel({
	...
	readiness: Awaited<ReturnType<typeof getCareerProfile>>["readiness"];
}) {
	const steps = [
	...
	return (
		<Card
			className={cn("overflow-hidden", readiness.ready && "border-signal/40")}
		>
						...
						{steps.map(([label, ready]) => (
							<div
								...
								className={cn(
									...
									ready
										? "border-signal/30 bg-signal/8 text-signal-foreground"
...
function AddPanel({
	...
	return (
		<details
			open={open}
			className="group rounded-lg border border-border bg-card shadow-[0_1px_2px_rgba(28,25,20,0.04)]"
		>
...
export default async function CareerProfilePage() {
	...
	const activeExperiences = profile.experiences.filter(
		(item) =>
			item.verificationState !== "archived" &&
			item.verificationState !== "prohibited",
	);
	const activeAchievements = profile.achievements.filter(
		(item) =>
			item.verificationState !== "archived" &&
			item.verificationState !== "prohibited",
	);
	...
	const healthWarnings = calculateEvidenceHealth(
		profile.experiences,
	...
	return (
		<div className="mx-auto w-full max-w-6xl px-5 py-8 sm:px-8 sm:py-12 lg:px-12 lg:py-14">
			...
			<header className="mb-8 grid border-b border-border pb-8 ">
				<h1 className="text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">
					Career profile
				</h1>
				<p className="max-w-2xl mt-3 text-sm leading-6 text-muted-foreground sm:text-base">
					Keep the evidence behind every resume claim in one private, reviewable
			...
			<Card
				className={cn(
					"mt-4 gap-4 py-5",
					healthWarnings.length && "border-amber-600/30",
				)}
			>
				<CardHeader className="grid gap-3 px-5 sm:grid-cols-[1fr_auto] sm:px-6">
					<div>
						...
						<p className="mt-1 text-sm leading-6 text-muted-foreground">
							Potential conflicts, duplicates, stale claims, and unsupported
							metrics are surfaced for review.
						</p>
					</div>
					<Badge
						variant={healthWarnings.length ? "outline" : "signal"}
						className={cn(
							healthWarnings.length && "border-amber-600/30 text-amber-800",
						)}
					>
						{healthWarnings.length ? <AlertTriangle /> : <Check />}
						{healthWarnings.length
							? `${healthWarnings.length} to review`
							: "No issues found"}
					</Badge>
				</CardHeader>
				{healthWarnings.length ? (
			...
			<section className="mt-10">
				<div className="mb-4">
					...
					<p className="mt-1 text-sm leading-6 text-muted-foreground">
						Start with what you know. You can verify and lock each item when it
						is accurate.
					</p>
				</div>
				<div className="space-y-3">
					<AddPanel
						...
						description="Record a role, its scope, dates, and working context."
						open={profile.experiences.length === 0}
					>
						<ExperienceForm />
					</AddPanel>
					<AddPanel
						title="Add achievement"
						description="Capture reusable evidence as problem, personal action, and result."
						open={
					...
					>
						{activeExperiences.length ? (
							<AchievementForm experiences={activeExperiences} />
						) : (
							<p className="text-sm text-muted-foreground">
								Add an active experience before linking an achievement.
							</p>
						)}
					</AddPanel>
					<AddPanel
						title="Add skill"
						description="Include proficiency, recency, context, and supporting evidence when available."
						open={
							profile.achievements.length > 0 && profile.skills.length === 0
						}
					>
				...
				<div className="mb-4 flex items-end justify-between gap-4">
					<div>
						...
						<p className="mt-1 text-sm text-muted-foreground">
							Employment history and the context behind your work.
						</p>
					</div>
					<span className="font-mono text-xs text-muted-foreground">
						{profile.experiences.length} total
					</span>
				</div>
							...
							>
								<CardHeader className="grid gap-4 px-5 sm:grid-cols-[1fr_auto] sm:px-6">
									<div>
										...
										<p className="mt-1 text-sm text-muted-foreground">
											{item.company} · {monthLabel(item.startDate)}–
								...
								<CardContent className="space-y-4 px-5 sm:px-6">
									...
									<Editor label="Edit experience" locked={item.locked}>
			...
			<section className="mt-12">
				<div className="mb-4 flex items-end justify-between gap-4">
					<div>
						...
						<p className="mt-1 text-sm text-muted-foreground">
							Specific proof you can reuse without inventing or overstating
							claims.
						</p>
					</div>
					<span className="font-mono text-xs text-muted-foreground">
						{profile.achievements.length} total
					</span>
				</div>
				{profile.achievements.length ? (
									...
									<CardContent className="space-y-4 px-5 sm:px-6">
										<div className="grid gap-4 rounded-md border border-border bg-muted/25 p-4 lg:grid-cols-3">
											<div>
												...
												<p className="mt-1 text-sm leading-6">{item.problem}</p>
											</div>
											<div>
												<p className="text-xs font-medium text-muted-foreground">
													Action
											...
											<div>
												...
												<p className="mt-1 text-sm leading-6">{item.result}</p>
											</div>
										</div>
										...
										<div className="border-t border-border pt-4">
											<EvidenceControls item={item} kind="career_achievement" />
										</div>
										<Editor label="Edit achievement" locked={item.locked}>
											<AchievementForm
												achievement={item}
					...
					<Card>
						<CardContent className="py-10 text-center text-sm text-muted-foreground">
							Achievements linked to your experience will appear here.
						</CardContent>
					</Card>
				)}
			</section>
			...
			<section className="mt-12">
				<div className="mb-4 flex items-end justify-between gap-4">
					<div>
						...
						<p className="mt-1 text-sm text-muted-foreground">
							Capabilities with enough context to judge where and how you used
							them.
						</p>
					</div>
					<span className="font-mono text-xs text-muted-foreground">
				...
				{profile.skills.length ? (
					<div className="grid gap-3 lg:grid-cols-2">
						{profile.skills.map((item) => (
							...
							>
								...
								<CardContent className="space-y-4 px-5 sm:px-6">
									{item.context ? (
										<p className="text-sm leading-6">{item.context}</p>
									) : null}
									<Provenance item={item} />
									<div className="border-t border-border pt-4">
										<EvidenceControls item={item} kind="career_skill" />
									</div>
									<Editor label="Edit skill" locked={item.locked}>
										<SkillForm skill={item} achievements={activeAchievements} />
									</Editor>
								</CardContent>
							</Card>
						))}
					...
					<Card>
						<CardContent className="py-10 text-center text-sm text-muted-foreground">
							Skills will appear here with recency and proficiency context.
						</CardContent>
					</Card>
				)}
			</section>
