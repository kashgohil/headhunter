...
export function FormFeedback({ state }: { state: CareerProfileActionState }) {
  return (
    <p
      ...
      className={cn(
        "min-h-5 text-sm",
        state.success ? "text-signal-foreground" : "text-destructive",
      )}
    >
      {state.message}
    </p>
...
export function useResetOnSuccess(
  state: CareerProfileActionState,
  editing: boolean,
) {
      ...
      <div className="grid gap-x-4 sm:grid-cols-2">
        ...
        >
          <Textarea
            ...
            placeholder="Scope, responsibilities, and the kind of problems you owned."
            className="min-h-24"
        ...
        >
          <Textarea
            ...
            placeholder={
              "Led discovery for the core workflow\nOwned the design system roadmap"
            }
            className="min-h-24"
            aria-invalid={Boolean(state.errors?.responsibilities)}
        ...
        >
          <Input
            id="sourceLabel"
            name="sourceLabel"
            defaultValue={experience?.sourceLabel ?? ""}
            placeholder="Resume 2026 or personal recollection"
            aria-invalid={Boolean(state.errors?.sourceLabel)}
            aria-describedby="sourceLabel-error"
          />
        </Field>
      </div>
  ...
  return (
    <form ref={formRef} action={formAction} className="space-y-4">
      <Field
        ...
        name="experienceId"
        error={state.errors?.experienceId}
      >
        <FormSelect
          name="experienceId"
          defaultValue={achievement?.experienceId ?? ""}
          required
          placeholder="Choose an experience"
          aria-invalid={Boolean(state.errors?.experienceId)}
          aria-describedby="experienceId-error"
      ...
      <div className="grid gap-x-4 lg:grid-cols-3">
        ...
        >
          <Textarea
            id="problem"
            name="problem"
            defaultValue={achievement?.problem}
            placeholder="What needed to change, and why did it matter?"
            className="min-h-32"
            required
            aria-invalid={Boolean(state.errors?.problem)}
        ...
        <Field label="Your action" name="action" error={state.errors?.action}>
          <Textarea
            id="action"
            name="action"
            defaultValue={achievement?.action}
            placeholder="What did you personally decide, build, or lead?"
            className="min-h-32"
            required
        ...
        <Field label="Result" name="result" error={state.errors?.result}>
          <Textarea
            ...
            placeholder="What changed for customers or the business?"
            className="min-h-32"
            required
      ...
      <div className="grid gap-x-4 sm:grid-cols-2">
        <Field
          ...
          error={state.errors?.measurableOutcome}
        >
          <Input
            id="measurableOutcome"
            name="measurableOutcome"
            defaultValue={achievement?.measurableOutcome ?? ""}
            placeholder="Reduced time-to-complete by 28%"
            aria-invalid={Boolean(state.errors?.measurableOutcome)}
            aria-describedby="measurableOutcome-error"
          />
        </Field>
        <Field
        ...
        >
          <Input
            id="tools"
            name="tools"
            defaultValue={achievement?.tools.join(", ")}
            placeholder="Research, prototyping, SQL"
            aria-invalid={Boolean(state.errors?.tools)}
        ...
        >
          <Input
            id="roleFamilies"
            name="roleFamilies"
            defaultValue={achievement?.roleFamilies.join(", ")}
            placeholder="Product design, Design systems"
            aria-invalid={Boolean(state.errors?.roleFamilies)}
            aria-describedby="roleFamilies-error"
          />
        </Field>
        <Field
          label="Source"
        ...
        >
          <Input
            id="sourceLabel"
            name="sourceLabel"
            defaultValue={achievement?.sourceLabel ?? ""}
            placeholder="Performance review or project notes"
            aria-invalid={Boolean(state.errors?.sourceLabel)}
            aria-describedby="sourceLabel-error"
          />
        </Field>
    ...
    <form ref={formRef} action={formAction} className="space-y-4">
      <div className="grid gap-x-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Field label="Last used" name="recency" error={state.errors?.recency}>
          <FormSelect
            name="recency"
            defaultValue={skill?.recency ?? "current"}
            options={[
              { value: "current", label: "Using currently" },
              { value: "recent", label: "Used in the last 2 years" },
              { value: "past", label: "Used earlier" },
            ]}
          />
        </Field>
        <Field
          label="Proficiency"
          name="proficiency"
          error={state.errors?.proficiency}
        >
          <FormSelect
            name="proficiency"
            defaultValue={skill?.proficiency ?? "working"}
            options={[
        ...
        >
          <Input
            id="sourceLabel"
            name="sourceLabel"
            defaultValue={skill?.sourceLabel ?? ""}
            placeholder="Project or resume"
            aria-invalid={Boolean(state.errors?.sourceLabel)}
            aria-describedby="sourceLabel-error"
          />
        </Field>
        ...
        >
          <Textarea
            ...
            placeholder="The scope and situations where you have applied this skill."
            className="min-h-24"
        ...
        >
          <FormSelect
            name="supportingAchievementId"
            defaultValue={skill?.supportingAchievementId ?? ""}
            options={[
              { value: "", label: "No linked achievement yet" },
              ...achievements.map((item) => ({
                value: item.id,
                label: item.result.slice(0, 80),
              })),
            ]}
          />
        </Field>
