...
function StageControl({ jobId, stage, stages }: { jobId: string; stage: string; stages: WorkspaceData["stages"] }) {
  ...
  return (
    <Popover>
      ...
      <PopoverContent align="end" className="max-h-(--radix-popover-content-available-height) w-80 overflow-y-auto">
...
function DocumentsTab({ jobId, data }: { jobId: string; data: WorkspaceData }) {
  ...
  return <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
    ...
    <Card className="h-fit"><CardHeader><CardTitle>Add document</CardTitle><p className="text-sm leading-6 text-muted-foreground">Store cover-letter text, a portfolio selection, or an attachment reference. Ready items are included in the submission snapshot.</p></CardHeader><CardContent><form action={action} className="space-y-4">
      <div><label htmlFor="artifact-kind" className="text-sm font-medium">Type</label><Select name="kind" defaultValue="cover_letter"><SelectTrigger id="artifact-kind" className="mt-2"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(artifactLabels).map(([value,label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div>
      ...
      <div><label htmlFor="artifact-state" className="text-sm font-medium">State</label><Select name="status" defaultValue="draft"><SelectTrigger id="artifact-state" className="mt-2"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="draft">Draft</SelectItem><SelectItem value="ready">Ready</SelectItem></SelectContent></Select></div>
...
function AnswersTab({ jobId, data }: { jobId: string; data: WorkspaceData }) {
  ...
  return <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
    ...
    <Card className="h-fit"><CardHeader><CardTitle>Add screening answer</CardTitle><p className="text-sm leading-6 text-muted-foreground">A library answer is copied here so job-specific edits remain isolated.</p></CardHeader><CardContent><form action={action} className="space-y-4">
      <div><label htmlFor="answer-library" className="text-sm font-medium">Answer library</label><Select name="canonicalAnswerId" defaultValue="new"><SelectTrigger id="answer-library" className="mt-2"><SelectValue placeholder="Start from approved text" /></SelectTrigger><SelectContent><SelectItem value="new">Write a new answer</SelectItem>{data.libraryAnswers.map((item) => <SelectItem key={item.id} value={item.id}>{item.question}</SelectItem>)}</SelectContent></Select></div>
...
function OutreachTab({ jobId, data }: { jobId: string; data: WorkspaceData }) {
  ...
  return <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
    ...
    <Card className="h-fit"><CardHeader><CardTitle>Draft outreach</CardTitle><p className="text-sm leading-6 text-muted-foreground">Nothing is sent from this workspace.</p></CardHeader><CardContent><form action={action} className="space-y-4">
      <div><label htmlFor="outreach-purpose" className="text-sm font-medium">Purpose</label><Select name="kind" defaultValue="recruiter_outreach"><SelectTrigger id="outreach-purpose" className="mt-2"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(outreachLabels).map(([value,label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div>
    ...
    <section className="mt-8" aria-labelledby="application-workspace-heading">
      <Card className="gap-0 overflow-hidden py-0">
        ...
        <Tabs defaultValue="plan" className="gap-0">
          ...
          <div className="bg-muted/20 p-4 sm:p-6"><TabsContent forceMount className="data-[state=inactive]:hidden" value="plan"><PlanTab jobId={jobId} data={data}/></TabsContent><TabsContent forceMount className="data-[state=inactive]:hidden" value="documents"><DocumentsTab jobId={jobId} data={data}/></TabsContent><TabsContent forceMount className="data-[state=inactive]:hidden" value="answers"><AnswersTab jobId={jobId} data={data}/></TabsContent><TabsContent forceMount className="data-[state=inactive]:hidden" value="outreach"><OutreachTab jobId={jobId} data={data}/></TabsContent><TabsContent forceMount className="data-[state=inactive]:hidden" value="submit"><SubmitTab jobId={jobId} data={data}/></TabsContent><TabsContent forceMount className="data-[state=inactive]:hidden" value="timeline"><TimelineTab jobId={jobId} data={data}/></TabsContent></div>
        </Tabs>
      </Card>
    </section>
  );
