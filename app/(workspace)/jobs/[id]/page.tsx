...
export default async function JobDetailPage(props: PageProps<"/jobs/[id]">) {
  ...
  const [job, fitAnalysis, profile, research, application, contacts] =
    await Promise.all([
      getJob(id),
  ...
  const evidenceReferences = [
    ...profile.experiences.map((item) => ({
      ...
      href: `/career-profile#evidence-${item.id}`,
    })),
    ...
    ...profile.profileItems.map((item) => ({
      id: item.id,
      label: item.title,
      href: `/career-profile/library#evidence-${item.id}`,
    })),
    ...
    <div className="mx-auto w-full max-w-6xl px-5 py-8 sm:px-8 sm:py-12 lg:px-12 lg:py-14">
          ...
          <div>
            ...
            <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-sm text-muted-foreground">
              ...
              <span className="flex items-center gap-2">
                ...
                {dateFormatter.format(job.capturedAt)}
              </span>
              <span className="flex items-center gap-2">
                <ShieldCheck className="size-3.5" /> Source preserved
              </span>
            </div>
          </div>
          {job.sourceUrl ? (
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold">
                Possible duplicate{" "}
                {job.duplicateJobs.length === 1 ? "job" : "jobs"}
              </h2>
              <p className="mt-1 text-xs leading-5 text-muted-foreground">
                Nothing was merged or removed. Compare the existing capture
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {job.duplicateJobs.map((duplicate) => (
                  <Button
                    ...
                    size="sm"
                  >
                    <Link href={`/jobs/${duplicate.id}`}>
                      {duplicate.company} · {duplicate.title}{" "}
      ...
      <section className="mt-8 rounded-lg border bg-card p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-lg font-semibold">Contacts & referrals</h2>
        ...
        {contacts.length ? (
          <ul className="mt-4 space-y-2">
            {contacts.map((contact) => (
              <li key={contact.id}>
                <Link
                  href={`/contacts/${contact.id}#opportunity-${job.id}`}
                ...
                <span className="ml-3 text-xs text-muted-foreground">
                  ...
                  {contact.referralStatus.replaceAll("_", " ")}
                </span>
              </li>
            ))}
          </ul>
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            No linked contacts yet. Link a contact to this role to track
            introductions and promised follow-ups.
          </p>
        )}
      </section>
        ...
        <ApplicationWorkspace
          ...
          data={{
            opportunity: {
              ...
              nextActionDueAt:
                application.opportunity.nextActionDueAt?.toISOString() ?? null,
            ...
            tasks: application.tasks.map((item) => ({
              ...item,
            ...
            artifacts: application.artifacts.map((item) => ({
              ...item,
              createdAt: undefined,
              updatedAt: item.updatedAt.toISOString(),
            })),
            outreach: application.outreach.map((item) => ({
              ...item,
              createdAt: undefined,
              updatedAt: item.updatedAt.toISOString(),
            })),
            submissions: application.submissions.map((item) => ({
              ...item,
              createdAt: undefined,
            ...
            events: [
              ...
              })),
              {
                ...
                kind: "note",
                title: "Job captured",
                detail: "Original source preserved",
                occurredAt: job.capturedAt.toISOString(),
              },
            ],
            libraryAnswers: application.libraryAnswers.map((item) => ({
              id: item.id,
              question: item.question,
              answer: item.answer,
              contexts: item.contexts,
            })),
            resumes: application.resumes.map((item) => ({
              ...item,
            ...
            stages: application.stages.map((item) => ({
              key: item.key,
              label: item.label,
      ...
      <Card className="mt-8">
        <CardHeader className="border-b border-border pb-6">
          <CardTitle>Structured job details</CardTitle>
          <p className="text-sm leading-6 text-muted-foreground">
            Review the extracted fields and correct anything the source
      ...
      <div className="grid gap-6 pt-8 lg:grid-cols-[minmax(0,1fr)_260px]">
        <Card>
          ...
          <CardContent>
            <div className="whitespace-pre-wrap font-mono text-[13px] leading-6 text-foreground/80">
              {job.originalDescription ||
                "No source text was captured for this manually entered role."}
            </div>
          </CardContent>
        </Card>
        ...
        <aside className="space-y-4">
          <Card className="gap-4">
            <CardHeader>
              <CardTitle className="leading-5">Source record</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4 text-xs leading-5 text-muted-foreground">
              <div>
                <p className="font-medium text-foreground">Capture method</p>
                <p className="mt-1 capitalize">
                  {job.sourceType === "url" ? "Imported URL" : job.sourceType}
                </p>
              </div>
              <div>
                <p className="font-medium text-foreground">Extraction</p>
                <Badge variant="outline" className="mt-1">
                  {confidenceLabel[job.extractionConfidence]}
                </Badge>
              </div>
              {job.sourceFetchedAt ? (
                <div>
                  ...
                  <p className="mt-1">
                    {dateFormatter.format(job.sourceFetchedAt)}
                  </p>
                </div>
              ) : null}
              {job.metadataUpdatedAt ? (
                <div>
                  <p className="font-medium text-foreground">Last corrected</p>
                  <p className="mt-1">
                    {dateFormatter.format(job.metadataUpdatedAt)}
                  </p>
                </div>
              ) : null}
            </CardContent>
          ...
          <div className="rounded-lg border border-dashed border-border p-4 text-xs leading-5 text-muted-foreground">
            Extracted values are suggestions. Saving corrections never changes
            the preserved source above.
          </div>
        </aside>
      </div>
    </div>
