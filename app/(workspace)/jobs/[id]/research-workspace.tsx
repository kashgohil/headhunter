...
function CompanyResearchForm({ jobId }: { jobId: string }) {
  ...
  const [state, formAction, pending] = useActionState(
    createCompanyResearchAction.bind(null, jobId),
    initialCompanyState,
  );
  ...
  return (
    <Disclosure
      className="rounded-lg border border-dashed border-border bg-background/55 px-4"
      triggerClassName="items-center gap-3 py-3 text-sm font-medium"
      title={
        <>
          <span className="flex items-center gap-2">
            <Plus className="size-4" /> Add company research
          </span>
    ...
    >
      ...
      >
        <div>
          <label className="text-sm font-medium" htmlFor="research-topic">
            Topic
          </label>
          <Select name="topic" defaultValue="product">
            <SelectTrigger id="research-topic" className="mt-2">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div>
          <label className="text-sm font-medium" htmlFor="research-provenance">
            Information type
          </label>
          <Select name="provenance" defaultValue="user_note">
            <SelectTrigger id="research-provenance" className="mt-2">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div className="sm:col-span-2">
          <label className="text-sm font-medium" htmlFor="research-content">
            Research note
          </label>
          <Textarea
            id="research-content"
            ...
            placeholder="What did you learn, observe, or infer?"
            aria-invalid={Boolean(state.errors?.content)}
          />
          <FieldError messages={state.errors?.content} />
        </div>
        <div className="sm:col-span-2">
          <label className="text-sm font-medium" htmlFor="research-url">
            Source URL
          </label>
          <Input
            id="research-url"
          ...
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
            Required for sourced facts. Optional for your notes and inferences.
          </p>
        ...
        <div>
          <label
            ...
            htmlFor="research-source-state"
          >
            Source state
          </label>
          <Select name="sourceState" defaultValue="unlinked">
            <SelectTrigger id="research-source-state" className="mt-2">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="unlinked">No linked source</SelectItem>
              {Object.entries(sourceStateLabels).map(([value, label]) => (
                <SelectItem key={value} value={value}>
                  {label}
                </SelectItem>
              ))}
        ...
        <div>
          <label className="text-sm font-medium" htmlFor="research-accessed-at">
            Accessed
          </label>
          <DatePicker
            id="research-accessed-at"
            name="accessedAt"
            className="mt-2"
          />
          ...
          <p
            className={
              state.errors
                ...
                : "text-sm text-signal-foreground"
            }
            aria-live="polite"
          >
            {state.message}
...
function SourceStateControl({
  ...
  return (
    <form
      action={updateResearchSourceStateAction.bind(null, jobId, entry.id)}
...
function CompanyEntryCard({
  ...
  const unhealthySource =
    entry.sourceState === "stale" || entry.sourceState === "inaccessible";

  return (
    <article
      className={
        unhealthySource
          ? "rounded-lg border border-destructive/25 bg-destructive/5 p-4"
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {entry.sourceState ? (
            <Badge variant={unhealthySource ? "secondary" : "signal"}>
              {sourceStateLabels[entry.sourceState]}
            </Badge>
          ) : null}
        </div>
          ...
          >
            ...
            {entry.accessedAt ? (
              <span className="shrink-0 font-normal text-muted-foreground">
                · accessed {dateFormatter.format(new Date(entry.accessedAt))}
              </span>
            ) : null}
          </a>
...
function OpportunityNoteForm({
  ...
  const [state, formAction, pending] = useActionState(
    saveOpportunityResearchAction.bind(null, jobId),
  ...
  return (
    <form action={formAction} className="space-y-4">
      <Textarea
        ...
        placeholder="Role-specific team signals, questions to resolve, application angles, and interview context…"
      />
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p
          className={
            state.error
              ? "text-sm text-destructive"
              : "flex items-center gap-2 text-sm text-signal-foreground"
          }
          aria-live="polite"
        >
          {state.message ? (
            <>
              {state.error ? null : <Check className="size-4" />}
              {state.message}
            </>
          ) : null}
        </p>
    ...
    <section className="mt-8" aria-labelledby="research-heading">
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-muted-foreground">
            Keep durable company knowledge separate from notes for this
            application.
          </p>
        </div>
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <Library className="size-3.5" /> {entries.length} reusable{" "}
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]">
        <Card>
          <CardHeader className="border-b border-border pb-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                <CardTitle>{company} library</CardTitle>
                <p className="mt-1 text-sm leading-6 text-muted-foreground">
                  Facts, notes, inferences, contacts, and open questions shared
                  across every role at this company.
                </p>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            {entries.length ? (
              ...
              <div className="grid min-h-32 place-items-center rounded-lg border border-dashed border-border px-6 text-center">
                <div>
                  ...
                  <p className="mt-1 text-xs leading-5 text-muted-foreground">
                    Add the first reusable signal for {company}.
                  </p>
                </div>
              </div>
            )}
        ...
        <Card className="self-start">
          <CardHeader className="border-b border-border pb-5">
            <div className="flex items-start gap-3">
              <FileQuestion className="mt-0.5 size-4 text-primary" />
              <div>
                <CardTitle>This opportunity</CardTitle>
                <p className="mt-1 text-sm leading-6 text-muted-foreground">
                  Private working notes for this role only. They never overwrite
                  the company library.
                </p>
              </div>
            </div>
          ...
          <CardContent>
            <OpportunityNoteForm jobId={jobId} content={opportunityNote} />
          </CardContent>
        </Card>
      </div>
    </section>
