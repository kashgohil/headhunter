...
export default async function JobsPage() {
  const [jobs, fitAnalyses] = await Promise.all([
    listJobs(),
    listLatestFitAnalyses(),
  ]);
    ...
    <div className="mx-auto w-full max-w-6xl px-5 py-8 sm:px-8 sm:py-12 lg:px-12 lg:py-14">
      <header className="flex flex-col gap-6 border-b border-border pb-8 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-xl text-sm leading-6 text-muted-foreground sm:text-base">
            A quiet place for roles worth a closer look. Capture the source
            first; qualify it later.
          </p>
        </div>
      ...
      <section className="pt-8" aria-labelledby="inbox-heading">
        <div className="mb-4 flex items-center justify-between">
          <h2 id="inbox-heading" className="text-sm font-semibold">
            Unqualified roles
          </h2>
          ...
          <Card className="overflow-hidden border-dashed bg-card/55 py-0">
            <CardContent className="grid min-h-80 place-items-center px-6 py-14 text-center">
              <div className="max-w-sm">
                <span className="mx-auto mb-5 grid size-11 place-items-center rounded-lg border border-border bg-background shadow-sm">
                  <BriefcaseBusiness className="size-5" />
                </span>
                <h3 className="text-base font-semibold">Your inbox is clear</h3>
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  Start with one real role. We’ll preserve the original posting
                  before any analysis touches it.
                </p>
                <Button asChild variant="outline" className="mt-6">
                  <Link href="/jobs/new">
                    <Plus /> Capture first job
                  </Link>
                </Button>
              </div>
            </CardContent>
            ...
            {jobs.map((job) => {
              ...
              return (
                <Link
                  key={job.id}
                  href={`/jobs/${job.id}`}
                  className="group grid gap-4 px-5 py-5 outline-none hover:bg-muted/60 focus-visible:bg-muted/60 sm:grid-cols-[1fr_auto] sm:items-center sm:px-6"
                >
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {recommendation ? (
                        ...
                        >
                          ...
                          {analysis?.overriddenRecommendation
                            ? " · overridden"
                            : ""}
                        </Badge>
                      ) : (
                  ...
                  <div className="flex items-center justify-between gap-5 sm:justify-end">
                    ...
                    <span className="text-xs text-muted-foreground">
                      Captured {dateFormatter.format(job.capturedAt)}
                    </span>
